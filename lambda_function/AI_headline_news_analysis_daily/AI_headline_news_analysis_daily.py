...
def analyze_news(news_text: str) -> str:
    ...
    2. important_news : 挑出真正**重要的重點新聞**，請根據新聞潛在意義與可能影響，不僅是出現次數。
    3. sentiment : 給出今日新聞的**情緒分析**（正面／負面／中性），並說明你的判斷依據。
    4. potential_stocks_and_industries : 根據新聞內容，推論出**值得高度關注的潛力個股與產業**，並解釋推薦原因（例如：政策利多、產業趨勢、事件驅動等）。
    ...
    final_system_prompt = """
        ...
        2. 挑出真正**重要的重點新聞**（important_news），請根據新聞潛在意義與可能影響，不僅是出現次數。
        3. 給出今日新聞的**情緒分析**（sentiment）:（正面／負面／中性），並說明你的判斷依據。
        4. 根據新聞內容，推論出**值得高度關注的潛力個股與產業**（potential_stocks_and_industries），並解釋推薦原因（例如：政策利多、產業趨勢、事件驅動等）。
