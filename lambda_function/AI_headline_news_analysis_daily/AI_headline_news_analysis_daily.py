...
def analyze_news(news_text: str) -> str:
    ...
    欄位說明：
    1. summary : 提供當日新聞的**精簡摘要**。
    2. important_news : 挑出真正**重要的重點新聞**，請根據新聞潛在意義與可能影響，不僅是出現次數。(如果要以條列方式撰寫，請以數字條列並在每個條列結尾加上換行符號 \n，避免變成一整段文字）。
    3. sentiment : 給出今日新聞的**情緒分析**（正面／負面／中性），並說明你的判斷依據。必定要給出（正面／負面／中性）三擇一關鍵字。
    4. potential_stocks_and_industries : 根據新聞內容，推論出**值得高度關注的潛力個股與產業**，並解釋推薦原因（例如：政策利多、產業趨勢、事件驅動等）。(如果要以條列方式撰寫，請以數字條列並在每個條列結尾加上換行符號 \n，避免變成一整段文字）。
    ...
    final_system_prompt = """
        ...
        說明如下：
        1. 提供當日新聞的**精簡摘要**（summary）。
        2. 挑出真正**重要的重點新聞**（important_news），請根據新聞潛在意義與可能影響，不僅是出現次數。(如果要以條列方式撰寫，請以數字條列並在每個條列結尾加上換行符號 \n，避免變成一整段文字）。
        3. 給出今日新聞的**情緒分析**（sentiment）:（正面／負面／中性），並說明你的判斷依據。必定要給出（正面／負面／中性）三擇一關鍵字。
        4. 根據新聞內容，推論出**值得高度關注的潛力個股與產業**（potential_stocks_and_industries），並解釋推薦原因（例如：政策利多、產業趨勢、事件驅動等）。(如果要以條列方式撰寫，請以數字條列並在每個條列結尾加上換行符號 \n，避免變成一整段文字）。
