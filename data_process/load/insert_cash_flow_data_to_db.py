...
logging.basicConfig(
    ...
    level=logging.ERROR,  # 記錄錯誤級別及以上的所有訊息
...
def get_db_connection():
    ...
    try:
        ...
        logging.error(f"連接資料庫失敗: {e}") # 將錯誤寫入日誌
        print(f"連接資料庫失敗: {e}") # 仍然在控制台顯示
        return None
...
def get_s3_client():
    ...
    try:
        ...
        logging.error(f"建立 S3 客戶端失敗: {e}") # 將錯誤寫入日誌
        print(f"建立 S3 客戶端失敗: {e}") # 仍然在控制台顯示
        return None
...
def process_s3_object(s3_client, bucket_name, key, conn, cursor):
    ...
    try:
        ...
        if not all([stock_code, year_roc, quarter, data_fields]):
            logging.warning(f"S3 物件 {key} 缺少必要的欄位。跳過。") # 將警告寫入日誌
            print(f"警告: S3 物件 {key} 缺少必要的欄位。跳過。") # 仍然在控制台顯示
            return

        # 檢查股票代碼是否在映射表中
        if stock_code not in my_company_id_map:
            logging.warning(f"股票代碼 {stock_code} 不在 company_id 映射表中。跳過 {key}。") # 將警告寫入日誌
            print(f"警告: 股票代碼 {stock_code} 不在 company_id 映射表中。跳過 {key}。") # 仍然在控制台顯示
            return
        ...
        conn.commit()
        print(f"成功插入或跳過 {stock_code}_{year_roc}Q{quarter} 的現金流量資料。")

    except errors.UniqueViolation:
        conn.rollback() # 回滾事務以避免鎖定
        logging.info(f"資料 {key} 已存在資料庫中 (重複鍵)。跳過。") # 將資訊寫入日誌
        print(f"資料 {key} 已存在資料庫中 (重複鍵)。跳過。") # 仍然在控制台顯示
    except json.JSONDecodeError as e:
        logging.error(f"無法解析 S3 物件 {key} 的 JSON 內容: {e}") # 將錯誤寫入日誌
        print(f"錯誤: 無法解析 S3 物件 {key} 的 JSON 內容: {e}") # 仍然在控制台顯示
    except KeyError as e:
        logging.error(f"S3 物件 {key} 中缺少預期的 JSON 鍵: {e}") # 將錯誤寫入日誌
        print(f"錯誤: S3 物件 {key} 中缺少預期的 JSON 鍵: {e}") # 仍然在控制台顯示
    except Exception as e:
        conn.rollback() # 確保在發生其他錯誤時回滾
        logging.error(f"處理 S3 物件 {key} 時發生未知錯誤: {e}") # 將錯誤寫入日誌
        print(f"處理 S3 物件 {key} 時發生未知錯誤: {e}") # 仍然在控制台顯示
...
def main():
    ...
    try:
        ...
        if not found_objects: # 只有當沒有找到任何物件時才記錄
            logging.info(f"S3 桶 '{S3_BUCKET_NAME}' 中 '{S3_KEY_PREFIX}' 前綴下沒有找到物件。") # 將資訊寫入日誌
            print(f"S3 桶 '{S3_BUCKET_NAME}' 中 '{S3_KEY_PREFIX}' 前綴下沒有找到物件。") # 仍然在控制台顯示

    except Exception as e:
        logging.error(f"主程式執行錯誤: {e}") # 將錯誤寫入日誌
        print(f"主程式執行錯誤: {e}") # 仍然在控制台顯示
    finally:
        if conn:
            conn.close()
            print("資料庫連接已關閉。")
