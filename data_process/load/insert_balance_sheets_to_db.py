...
# 設定日誌檔路徑在程式碼相同資料夾內
log_file_path = os.path.join(current_dir, 'balance_sheet_processing.log')

logging.basicConfig(
    level=logging.ERROR, # 只記錄 ERROR 及以上級別的訊息
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename=log_file_path,
    filemode='a' # 'a' 表示追加模式，'w' 表示寫入模式 (會覆蓋)
)
# 可以再設定一個 StreamHandler 讓錯誤同時輸出到控制台
console_handler = logging.StreamHandler()
console_handler.setLevel(logging.ERROR) # 控制台也只顯示 ERROR 級別
...
# 值為 (資料庫數字欄位, 資料庫百分比欄位, [S3 JSON原始鍵])
# 嚴格依照你提供的 S3 到資料庫欄位對應來寫入
DATA_FIELD_MAPPING = {
...
def download_json_from_s3(bucket_name, s3_key, local_path, access_key, secret_key):
    ...
    try:
        ...
        logging.error(f"從 S3 下載 {s3_key} 失敗: {e}", exc_info=True)
        print(f"從 S3 下載 {s3_key} 失敗，詳情請參考日誌。")
        return False

def parse_filename(filename):
    """
    解析 S3 檔案名稱，提取股票代碼、年份、季度和資料類型。
    例如: '1101_104Q1_balance_sheet.json'
    """
    match = re.match(r'(\d+)_(\d+)Q([1-4])_(balance_sheet)\.json', filename)
    if match:
        stock_code = match.group(1)
        year_roc = int(match.group(2))
        quarter = int(match.group(3))
        report_kind = match.group(4)
        data_type_from_filename = 'quarterly' # 資產負債表預設為 quarterly
...
def process_s3_financial_statements(s3_key_prefix, company_id_map, access_key, secret_key):
    ...
    if not company_id_map:
        logging.error("公司ID映射為空，無法處理財報資料。請提供有效的 company_id_map。")
        print("公司ID映射為空，無法處理財報資料。")
        return
    ...
    try:
        ...
        for obj in objects:
            ...
            if not all([stock_code, year_roc, quarter, report_kind, data_type_from_filename]):
                logging.error(f"錯誤: 無法解析檔案名稱 {filename}。跳過。")
            ...
            if download_json_from_s3(S3_BUCKET_NAME, s3_key, local_json_path, access_key, secret_key):
                try:
                    ...
                    transformed_record = transform_balance_sheet_data(json_data,
                                                                         (stock_code, year_roc, quarter, report_kind, data_type_from_filename),
                                                                         company_id_map)

                    if transformed_record:
                        insert_balance_sheet_to_db(DB_CONFIG, transformed_record)
                    else:
                        logging.error(f"檔案 {s3_key} 的資料轉換失敗。")
                        print(f"檔案 {s3_key} 的資料轉換失敗，詳情請參考日誌。")

                except json.JSONDecodeError as e:
                    logging.error(f"檔案 {local_json_path} 不是有效的 JSON 格式: {e}", exc_info=True)
                    print(f"檔案 {local_json_path} 不是有效的 JSON 格式，詳情請參考日誌。")
                except Exception as e:
                    logging.error(f"處理檔案 {local_json_path} 時發生未預期的錯誤: {e}", exc_info=True)
                    print(f"處理檔案 {local_json_path} 時發生未預期的錯誤，詳情請參考日誌。")
                finally:
        ...
        logging.error(f"列出或處理 S3 物件時發生錯誤: {e}", exc_info=True)
        print(f"列出或處理 S3 物件時發生錯誤，詳情請參考日誌。")
...
if __name__ == "__main__":
    ...
    if not AWS_ACCESS_KEY_ID or not AWS_SECRET_ACCESS_KEY:
        logging.critical("錯誤：AWS_ACCESS_KEY_ID 或 AWS_SECRET_ACCESS_KEY 環境變數未設置。請檢查 .env 檔案或環境配置。")
        print("錯誤：AWS_ACCESS_KEY_ID 或 AWS_SECRET_ACCESS_KEY 環境變數未設置。請檢查 .env 檔案或環境配置。")
    else:
        process_s3_financial_statements(
            s3_key_prefix='raw-data/balance-sheet/',
