...
OUTPUT_MISSING_FILE_PATH = 'missing_income_statement_data.json'

QUARTERS = [1, 2, 3, 4]
# 損益表的類型：累計和季度
INCOME_STATEMENT_TYPES = ['accumulated', 'quarterly']
...
def parse_filename(filename, prefix):
    """從 S3 物件鍵中解析出股票代碼、年份、季度和報表類型"""
    # 移除前綴
    if filename.startswith(prefix):
        filename = filename[len(prefix):]
    
    # 正規表達式：STOCKCODE_YYYYQ_income_statement_(accumulated|quarterly).json
    match = re.match(r'(\d+)_(\d{3,4})Q(\d)_income_statement_(accumulated|quarterly)\.json', filename)
    if match:
        stock_code = match.group(1)
        year_roc = int(match.group(2))
        quarter = int(match.group(3))
        report_type = match.group(4)
        return stock_code, year_roc, quarter, report_type
    return None, None, None, None

def main():
    # --- 新增：使用 argparse 處理命令列參數 ---
    parser = argparse.ArgumentParser(description="檢查 S3 損益表數據的完整性。")
    ...
    # 格式: "stock_code_year_quarter_type"
    existing_data_set = set()
    for key in s3_object_keys:
        stock_code, year_roc, quarter, report_type = parse_filename(key, S3_RAW_DATA_PREFIX)
        if stock_code and year_roc and quarter and report_type:
            existing_data_set.add(f"{stock_code}_{year_roc}Q{quarter}_{report_type}")

    print("開始比對預期數據與 S3 中存在的數據...")
    # missing_data_by_company_year 格式: {stock_code: {year: {quarter: [missing_types, ...]}}}
    missing_data_by_company_year = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    ...
    for stock_code in all_companies.keys():
        for year_roc in range(start_year_roc, end_year_roc + 1):
            ...
            for quarter in current_quarters_to_check:
                types_to_check = []
                if quarter == 1 or quarter == 4: # Q1 和 Q4 只檢查 accumulated
                    types_to_check.append('accumulated')
                elif quarter == 2 or quarter == 3: # Q2 和 Q3 檢查 accumulated 和 quarterly
                    types_to_check.append('accumulated')
                    types_to_check.append('quarterly')

                for report_type in types_to_check:
                    expected_key_format = f"{stock_code}_{year_roc}Q{quarter}_{report_type}"
                    
                    if expected_key_format not in existing_data_set:
                        missing_data_by_company_year[stock_code][year_roc][f"Q{quarter}"].append(report_type)
    ...
    for stock_code, years_data in missing_data_by_company_year.items():
        ...
        for year_roc in sorted(years_data.keys()):
            sorted_quarters_data = {}
            for quarter_str in sorted(years_data[year_roc].keys(), key=lambda q: int(q.replace('Q', ''))):
                # 排序缺少的報表類型
                sorted_types = sorted(years_data[year_roc][quarter_str])
                sorted_quarters_data[quarter_str] = sorted_types
                total_missing_count += len(sorted_types)
            sorted_years_data[str(year_roc)] = sorted_quarters_data # 將年份鍵轉換為字串
        formatted_missing_data[stock_code] = sorted_years_data


    if total_missing_count == 0:
        print("\n太棒了！S3 中沒有發現任何缺少的損益表數據。")
    else:
        print(f"\n發現 {total_missing_count} 筆缺少的損益表數據。")
