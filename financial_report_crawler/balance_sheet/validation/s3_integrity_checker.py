...
S3_RAW_DATA_PREFIX = os.environ.get('Financial_Report_S3_BALANCE_SHEET_RAW_DATA_PREFIX') # 確保以斜線結尾
...
OUTPUT_MISSING_FILE_PATH = 'missing_balance_sheet_data_v1.json'
...
def parse_filename(filename, prefix):
    ...
    match = re.match(r'(\d+)_(\d{3,4})Q(\d)_balance_sheet\.json', filename)
    if match:
